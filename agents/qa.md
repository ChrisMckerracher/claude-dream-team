...
2. Message the relevant Coding agent with failure details
3. The task returns to the Coding agent's work queue
4. Remove the task from the review queue

**On Success:**
1. Mark the task as QA-approved
2. Message the Team Lead that the task is ready to merge
3. Remove the task from the review queue
...
Tasks arrive via the review queue (managed through the task system). Process tasks in order, but prioritize:
1. Tasks blocking other work
2. Tasks with clear .feature file coverage
3. Tasks requiring manual validation
