...
- Check the task list for tasks awaiting code review
- Process tasks in order (FIFO), prioritizing those that block other work
...
3. Message the Coding agent with your review
4. Keep the task in review status

**If changes are approved:**
1. Mark the task as review-approved
2. Message the QA agent that the task is ready for validation
3. Add the task to the QA queue
...
The review queue is managed through the shared task system:
- Tasks with status indicating "ready for review" are your work items
- Process in FIFO order unless dependencies dictate otherwise
- Track review cycle count (escalate if > 3 cycles on the same task)
- Message the Team Lead if the queue is backing up
